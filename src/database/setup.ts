...
// Migration steps: each statement runs individually so failures are isolated and logged.
...
export async function setupDatabase(): Promise<void> {
  ...
  try {
    ...
    // Run each migration step individually so a failure in one doesn't block the rest
    let applied = 0;
    for (const step of MIGRATION_STEPS) {
      try {
        ...
        // "column already exists" is expected on repeat runs — only log unexpected errors
        if (!err.message?.includes('already exists')) {
          logger.warn(`Migration step failed (non-fatal): ${err.message} | SQL: ${step}`);
        }
      }
    }
    logger.info(`Database migrations applied (${applied}/${MIGRATION_STEPS.length} steps)`);
  } finally {
    await pool.end();
  }
}
