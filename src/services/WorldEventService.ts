...
export const WORLD_EVENTS: Record<string, WorldEvent> = {
  harvest_festival: {
    id: 'harvest_festival', name: 'Harvest Festival', emoji: '🎪', type: 'festival',
    description: 'The whole garden celebrates the season\'s bounty! Crops sell for double price and rare seeds appear in the shop.',
  ...
  moonstone_shower: {
    id: 'moonstone_shower', name: 'Moonstone Shower', emoji: '🌠', type: 'mystery',
    description: 'Moonstones rain from the sky! Find extras during exploration.',
  ...
  great_storm: {
    id: 'great_storm', name: 'Great Storm', emoji: '⛈️', type: 'storm',
    description: 'A fierce magical storm sweeps the garden. Rare wildlife emerge and plants grow faster.',
  ...
  shadow_invasion: {
    id: 'shadow_invasion', name: 'Shadow Invasion', emoji: '🌑', type: 'invasion',
    description: 'Shadow creatures invade the garden! Fight them back to protect your crops.',
  ...
  merchant_caravan: {
    id: 'merchant_caravan', name: 'Merchant Caravan', emoji: '🛒', type: 'market',
    description: 'A traveling merchant caravan arrives with exotic goods at reduced prices!',
...
export class WorldEventService {
  ...
  static async startEvent(guildId: string, eventId: string): Promise<ActiveWorldEvent> {
    ...
    await NewsService.postNews(guildId, `${event.emoji} ${event.name} Has Started!`, event.description, 'world_event', 3, event.durationHours + 1);
  ...
  static async maybeSpawnEvent(guildId: string): Promise<WorldEvent | null> {
    ...
    if (!chance(0.05)) return null; // 5% per tick
  ...
  static async participate(guildId: string, playerId: string): Promise<{ event: WorldEvent; alreadyIn: boolean }> {
    const active = await WorldEventService.getActiveEvent(guildId);
    if (!active) throw new Error('No active world event');
