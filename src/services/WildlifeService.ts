...
export const WILDLIFE: Record<string, Wildlife> = {
  twilight_fox: {
    id: 'twilight_fox', name: 'Cáo Hoàng Hôn', emoji: '🦊', rarity: 'uncommon',
    description: 'Một con cáo có bộ lông đổi màu lúc hoàng hôn.',
  ...
  moon_owl: {
    id: 'moon_owl', name: 'Cú Mặt Trăng', emoji: '🦉', rarity: 'rare',
    description: 'Con cú có đôi mắt chứa đựng những mảnh trăng nhỏ.',
  ...
  crystal_deer: {
    id: 'crystal_deer', name: 'Nai Pha Lê', emoji: '🦌', rarity: 'rare',
    description: 'Gạc làm bằng pha lê sống, phát ra tiếng leng keng trong gió.',
  ...
  glowworm: {
    id: 'glowworm', name: 'Sâu Phát Quang', emoji: '🪲', rarity: 'common',
    description: 'Con sâu nhỏ xíu phát ra ánh sáng sinh học lung linh.',
  ...
  starfish: {
    id: 'starfish', name: 'Sao Biển Lơ Lửng', emoji: '⭐', rarity: 'uncommon',
    description: 'Con sao biển bay lơ lửng trong không trung những đêm trăng sáng.',
  ...
  shadow_wolf: {
    id: 'shadow_wolf', name: 'Sói Bóng Tối', emoji: '🐺', rarity: 'epic',
    description: 'Con sói khổng lồ đi trong bóng tối, không để lại dấu chân.',
  ...
  phantom_butterfly: {
    id: 'phantom_butterfly', name: 'Bướm Ma', emoji: '🦋', rarity: 'rare',
    description: 'Con bướm trong suốt đến mức nhìn xuyên cánh có thể thấy các vì sao.',
  ...
  cave_drake: {
    id: 'cave_drake', name: 'Rồng Hang Động', emoji: '🐲', rarity: 'legendary',
    description: 'Rồng nhỏ làm tổ trong hang sâu và tích trữ pha lê.',
...
export class WildlifeService {
  ...
  static async tame(playerId: string, wildlifeId: string): Promise<{ success: boolean; message: string }> {
    ...
    if (!wildlife) throw new Error('Không tìm thấy sinh vật này.');
    ...
    for (const cost of wildlife.tameCost) {
      ...
      if (!hasItem) {
        ...
        throw new Error(`Bạn cần ${cost.quantity}x ${def?.name ?? cost.itemId} để thuần hóa sinh vật này.`);
      }
    }
    ...
    if (chance(wildlife.tameChance)) {
      ...
      return { success: true, message: `Bạn đã thuần hóa thành công **${wildlife.name}** ${wildlife.emoji}! Nó đã tin tưởng bạn.` };
    }

    return { success: false, message: `**${wildlife.name}** ${wildlife.emoji} kháng cự và chạy vào bóng tối.` };
  }
